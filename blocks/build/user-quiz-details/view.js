...
jQuery(document).ready($ => {
  ...
  const updateDateRangeText = () => {
    ...
    if (!dateFrom && !dateTo) $block.find('#date-range-text').text('Select a date range');else if (dateFrom && dateTo) $block.find('#date-range-text').text(`${dateFrom} - ${dateTo}`);else if (dateFrom) $block.find('#date-range-text').text(`From ${dateFrom}`);else $block.find('#date-range-text').text(`Until ${dateTo}`);
  };
  const fpFrom = (0,flatpickr__WEBPACK_IMPORTED_MODULE_4__["default"])($dateFrom[0], {
    ...FP_FILTER,
  ...
  const fpTo = (0,flatpickr__WEBPACK_IMPORTED_MODULE_4__["default"])($dateTo[0], {
    ...FP_FILTER,
    placeholder: 'Pick a date',
    onChange(_, dateStr) {
  ...
  const fpFor = new Map([[$dateFrom[0].id, fpFrom], [$dateTo[0].id, fpTo]]);
  ...
  const loadQuizData = async () => {
    ...
    try {
      const url = _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.endpoints.userQuizProgress(userId);

...
/* harmony import */ var _shared_alert_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../_shared/alert.js */ "./src/_shared/alert.js");
...
const FP_SHARED = {
  ...
  // prevent inheriting original's classes
  altFormat: 'j M Y, H:i',
  time_24hr: true,
  disableMobile: true,
  allowInput: false,
...
function initRowPickers(quizId, startEl, endEl, startVal, endVal, $rowSaveBtn) {
  ...
  startFp = (0,flatpickr__WEBPACK_IMPORTED_MODULE_4__["default"])(startEl, {
    ...FP_SHARED,
  ...
  endFp = (0,flatpickr__WEBPACK_IMPORTED_MODULE_4__["default"])(endEl, {
    ...FP_SHARED,
    placeholder: 'No date restriction',
    onChange(_, dateStr) {
...
function destroyRowPickers() {
  Object.values(rowPickerMap).forEach(({
    startFp,
    endFp
  }) => {
    startFp?.destroy();
...
function buildQuizRow(quiz, courseName, stat) {
  ...
  $row.find('.gqc__badge--completed').attr('data-quiz-id', quiz.step_id).attr('data-quiz-name', quiz.step_title).find('.gqc__badge-count').text(completed);
  $row.find('.gqc__badge--pending').attr('data-quiz-id', quiz.step_id).attr('data-quiz-name', quiz.step_title).find('.gqc__badge-count').text(outstanding);
  return $row;
}
...
async function loadQuizData($block, groupId, courses, readOnly = false) {
  ...
  try {
    ...
    const courseQuizData = courses.map(course => {
      ...
      const steps = Array.isArray(cached?.quizzes_show_test_grading_config) ? cached.quizzes_show_test_grading_config : [];
      return {
        course,
        steps
      };
    });
    const allQuizIds = courseQuizData.flatMap(({
      steps
    ...
    quizAccessDatesMap = {};
    courseQuizData.forEach(({
      steps
    }) => {
      steps.forEach(q => {
        quizAccessDatesMap[q.step_id] = {
          start: q.start || '',
          end: q.end || ''
        };
      });
    });
    const stats = await _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.get(_shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.endpoints.groupQuizSubmissionStats(groupId, allQuizIds));
    const statsMap = {};
    (Array.isArray(stats) ? stats : []).forEach(s => {
      statsMap[s.quiz_id] = s;
    });
    $skeleton.hide();
    courseQuizData.forEach(({
      course,
    ...
    $list.find('.gqc__item').each(function () {
      ...
      if (readOnly) {
        ...
        initRowPickers(quizId, $item.find('.gqc__datetime--start')[0], $item.find('.gqc__datetime--end')[0], (0,_shared_helpers_js__WEBPACK_IMPORTED_MODULE_2__.convertFromUTC)(dates.start || ''), (0,_shared_helpers_js__WEBPACK_IMPORTED_MODULE_2__.convertFromUTC)(dates.end || ''), $rowSaveBtn);
      }
    });
...
jQuery(document).ready($ => {
  ...
  async function persistRowAccessDates($item, quizId) {
    const start = (0,_shared_helpers_js__WEBPACK_IMPORTED_MODULE_2__.convertToUTC)($item.find('.gqc__datetime--start').val() || '');
    const end = (0,_shared_helpers_js__WEBPACK_IMPORTED_MODULE_2__.convertToUTC)($item.find('.gqc__datetime--end').val() || '');
    await _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.post(_shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.endpoints.groupQuizAccess(currentGroupId), {
      quiz_id: quizId,
      start,
      end
    });
    quizAccessDatesMap[quizId] = {
      start,
      end
    };
    ...
    if (Array.isArray(cachedCourses)) {
      const updated = cachedCourses.map(c => ({
        ...c,
        quizzes_show_test_grading_config: Array.isArray(c.quizzes_show_test_grading_config) ? c.quizzes_show_test_grading_config.map(q => q.step_id === quizId ? {
          ...q,
          start,
          end
        } : q) : []
      }));
      _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].setCourses(updated);
    }
    _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.invalidate('quiz-access');
    _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.invalidate('quiz-submission-stats');
    _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.invalidate('quiz-attempts');
  }
  $block.on('click', '.gqc__save-row', async function () {
    ...
    try {
      ...
      (0,_shared_alert_js__WEBPACK_IMPORTED_MODULE_3__.bysAlert)('Failed to save the quiz access window. Please try again.');
    }
  });
  ...
  $block.on('click', '.gqc__notify', async function () {
    ...
    try {
      await persistRowAccessDates($item, quizId);
      const result = await _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.post(`/wp-json/bys-groups/v1/groups/${currentGroupId}/quizzes/${quizId}/notify-access`);
      ...
      if (skipped > 0 && sent === 0) {
        ...
        (0,_shared_alert_js__WEBPACK_IMPORTED_MODULE_3__.bysAlert)(`All ${skipped} learner${skipped === 1 ? '' : 's'} in this group have turned off group communications. No notifications were sent.`);
        return;
      }
      ...
      if (skipped > 0) {
        (0,_shared_alert_js__WEBPACK_IMPORTED_MODULE_3__.bysAlert)(`${skipped} learner${skipped === 1 ? ' has' : 's have'} turned off group communications and did not receive this notification. Check the Communications log for per-recipient delivery status.`);
      }
    } catch (err) {
      ...
      (0,_shared_alert_js__WEBPACK_IMPORTED_MODULE_3__.bysAlert)('Failed to save the quiz access window and notify learners. Please try again.');
    }
  });
  ...
  $(document).on('bys:groupSelected', (_, {
    ...
    memberCount = _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getUsers()?.length ?? 0;
    const isSiteEditor = window.bysGroupsAuth?.isSiteEditor === true;
    loadQuizData($block, groupId, _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getCourses() || [], isSiteEditor);
  });
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && currentGroupId && memberCount > 0) {
      _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.invalidate('quiz-steps');
      _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.invalidate('quiz-submission-stats');
      _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.invalidate('quiz-attempts');
      loadQuizData($block, currentGroupId, _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getCourses() || []);
    }
  });

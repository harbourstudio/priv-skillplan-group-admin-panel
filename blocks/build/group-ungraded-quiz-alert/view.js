...
(() => {
/*!***********************************************!*\
  !*** ./src/group-ungraded-quiz-alert/view.js ***!
  \***********************************************/
__webpack_require__.r(__webpack_exports__);
...
jQuery(document).ready($ => {
  ...
  async function updateAlert(groupId, courses) {
    ...
    // attempts.
    const allQuizIds = courses.flatMap(c => Array.isArray(c.quizzes_show_test_grading_config) ? c.quizzes_show_test_grading_config : []).map(q => q.step_id);
    ...
    try {
      ...
      if (Array.isArray(statsArray)) {
        statsArray.forEach(s => {
          totalUngraded += s.ungraded_count || 0;
        });
      }
    } catch (err) {
  ...
  $(document).on('bys:groupSelected', function (_, data) {
    ...
    $count.text('');
    updateAlert(data.groupId, _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getCourses() || []);
  });

...
(function () {
  document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.wp-block-bys-groups-group-reporting').forEach(initBlock);
  });
  function initBlock(block) {
    ...
    function toggleCourseColumn(idx, expand) {
      ...
      if (expand) {
        ...
        primaryTh.parentElement.querySelectorAll(`.course-sub-th[data-course-idx="${idx}"]`).forEach(el => el.remove());
      }
      ...
      if (expand) {
        ...
        secondaryTd.parentElement.querySelectorAll(`.course-sub-dl[data-course-idx="${idx}"]`).forEach(el => el.remove());
      }

      // ── Body rows ────────────────────────────────────────────────
      table.querySelectorAll(`.course-cell[data-course-idx="${idx}"]`).forEach(td => {
        if (expand) {
          expandBodyCell(td);
        } else {
          collapseBodyCell(td);
    ...
    function expandBodyCell(td) {
      ...
      let quizzes = [];
      try {
        quizzes = JSON.parse(td.dataset.quizzes || '[]');
      } catch (e) {}

      // Progress bar sub-cell
      const color = progress >= 100 ? 'var(--wp--preset--color--green-500)' : progress > 0 ? 'var(--wp--preset--color--orange-500)' : 'var(--wp--preset--color--gray-300)';
      ...
      tdEnrol.innerHTML = enrolment ? enrolment : '<span class="bys-date-empty">Not Started</span>';
      ...
      tdComp.innerHTML = completion ? completion : '<span class="bys-date-empty">Not Completed</span>';
    ...
    function collapseBodyCell(td) {
      ...
      td.parentElement.querySelectorAll(`.course-cell--sub[data-course-idx="${td.dataset.courseIdx}"]`).forEach(el => el.remove());
      const progress = parseInt(td.dataset.progress, 10) || 0;
    ...
    function buildQuizIcons(quizzes) {
      ...
      return '<div class="bys-quiz-icons">' + quizzes.map(q => {
        const color = q.score >= 80 ? 'var(--wp--preset--color--green-500)' : q.score >= 50 ? 'var(--wp--preset--color--orange-500)' : 'var(--wp--preset--color--red-500)';

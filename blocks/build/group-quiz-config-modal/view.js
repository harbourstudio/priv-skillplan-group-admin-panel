...
(() => {
/*!*********************************************!*\
  !*** ./src/group-quiz-config-modal/view.js ***!
  \*********************************************/
__webpack_require__.r(__webpack_exports__);
...
jQuery(document).ready($ => {
  ...
  function buildUserRow(displayName, email, status, attemptCount) {
    ...
    const attemptsHtml = attemptCount > 0 ? `<span class="quiz-attempts-count">${attemptCount} ${attemptCount === 1 ? 'attempt' : 'attempts'}</span>` : `<span class="quiz-attempts-count quiz-attempts-count--none">—</span>`;
    return `
  ...
  async function loadQuizData(groupId, quizId, quizName) {
    ...
    renderSkeleton();
    try {
      const attempts = await _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.get(_shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.endpoints.groupQuizAttempts(groupId, quizId));
      ...
      const attempted = Object.values(bestByUser).filter(a => a.completed_gmt !== null).map(a => ({
        user_id: a.user_id,
      ...
      const allMemberIds = _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getUserIds() || [];
      ...
      if (notAttemptIds.length) {
        try {
          ...
          const cachedHydrated = _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getCurrentGroup() === Number(groupId) ? _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getHydratedUsers(notAttemptIds) : null;
          let users;
          if (cachedHydrated !== null) {
            ...
            users = await _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.get(_shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.endpoints.groupUsers(groupId, notAttemptIds.join(',')));
      ...
      $meta.html(`<span class="quiz-completed-count">${completedCount}</span> completed` + ` &bull; ` + `<span class="quiz-outstanding-count">${notAttemptedCount}</span> not attempted`);

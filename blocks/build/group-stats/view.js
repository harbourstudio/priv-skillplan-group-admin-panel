...
jQuery(document).ready($ => {
  ...
  $(document).on('bys:groupSelected', async (_, {
    ...
    const userIdsForStats = _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getUserIds() || [];
    const coursesForStats = _shared_store_js__WEBPACK_IMPORTED_MODULE_1__["default"].getCourses() || [];
  ...
  async function calculateCourseStats(userIds, courses) {
    try {
      ...
      const promises = userIds.map(userId => {
        ...
        return _shared_api_client_js__WEBPACK_IMPORTED_MODULE_0__.api.get(endpoint, false);
      });
